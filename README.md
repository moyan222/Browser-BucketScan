...
BucketTool-2.0 是一款轻量易用的浏览器扩展工具，专为云存储桶安全检测设计，适配主流浏览器环境（摆脱 BurpSuite 版本兼容限制），可全面检测**阿里云 OSS、腾讯云 COS、华为云 OBS、AWS S3、京东云 OSS、百度云 BOS、火山引擎 TOS** 七种主流云存储桶的核心安全漏洞。
...
- **多种导出格式**：支持 JSON、CSV、HTML 三种格式导出，满足不同场景需求
...
- **主动批量检测**：支持在日志窗口粘贴多行URL（每行一个），或导入 `.txt` 文件，自动逐个检测并实时显示结果。
  ...
  - **打开方式**：点击扩展图标，或右键点击扩展图标选择"用 BucketTool-2.0 检测"
  
  ![image-20251222111905593](./imgs/image-20251222111905593.png)
![image-20260510202731369](./imgs/image-20260510202731369.png)
  
  ![image-20260510203207392](./imgs/image-20260510203207392.png)
...
- 2025-12-20
  ...
  - 优化：JSON导出内容，包含完整数据包和元信息

  - 新增：导出功能支持JSON、CSV、HTML三种格式
